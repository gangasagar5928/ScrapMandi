...
export const CategoryGrid = ({ onSelectCategory, setActiveTab }) => {
  return (
    <div className="py-16 sm:py-20 bg-slate-900 text-white relative overflow-hidden border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {SCRAP_CATEGORIES.map((cat) => {
            ...
            return (
              <div
                ...
                className="group relative rounded-3xl overflow-hidden border border-slate-800 hover:border-emerald-500/60 shadow-xl transition cursor-pointer flex flex-col justify-between min-h-[280px] bg-slate-950"
              >
                ...
                <div className="relative z-10 p-6 flex items-center justify-between">
                  <div className="w-12 h-12 rounded-2xl bg-slate-900 text-emerald-400 border border-slate-700/80 flex items-center justify-center group-hover:border-emerald-500 transition-all">
                    <IconComponent className="w-6 h-6" />
                  </div>
                  <span className="text-xs font-bold text-slate-300 group-hover:text-emerald-400 flex items-center gap-1.5 bg-slate-900 px-3 py-1 rounded-full border border-slate-700">
                ...
                <div className="relative z-10 p-6 pt-0 space-y-3">
                  ...
                  <div className="pt-2 flex flex-wrap gap-1.5">
                    {cat.subcategories.slice(0, 3).map(sub => (
                      <span key={sub.id} className="text-[10px] bg-slate-900 border border-slate-700 text-slate-200 px-2 py-0.5 rounded-lg font-medium">
                        {sub.name}
                      </span>
                    ))}
        ...
        <div className="bg-slate-950 text-white rounded-3xl p-6 sm:p-12 shadow-2xl overflow-hidden relative border border-slate-800">
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 space-y-4 hover:border-emerald-500/50 transition">
                ...
                <button
                  ...
                  className="w-full mt-3 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black py-3 rounded-xl text-xs transition text-center shadow-lg shadow-emerald-600/20 cursor-pointer"
                >
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 space-y-4 hover:border-blue-500/50 transition">
                <div className="flex items-center gap-3">
                ...
                <ul className="space-y-3 text-xs text-slate-300">
                  <li className="flex items-start gap-2.5">
                    ...
                    <span><strong>Live Yard Inspection:</strong> View actual yard photos, purity grade, and location.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                ...
                <button
                  ...
                  className="w-full mt-3 bg-slate-800 hover:bg-slate-750 text-white font-bold py-3 rounded-xl text-xs border border-slate-700 transition text-center cursor-pointer"
                >
            ...
            <div className="mt-8 pt-8 border-t border-slate-800">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 text-center mb-4">
                Active Covered Industrial Clusters in Delhi NCR
              </h4>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {MAJOR_MANDIS.map((mandi) => (
                  <div key={mandi.id} className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-center">
                    <MapPin className="w-4 h-4 text-emerald-400 mx-auto mb-1" />
                    <p className="font-bold text-white text-xs">{mandi.name}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{mandi.type}</p>
                  </div>
                ))}
              </div>
            </div>
